...
{
    internal class SqliteConverter : IConverter
    {
    ...
    public List<SheetData> Convert(DataSet data)
        ...
        public DataTable ConvertSheet(DataTable sheet)
        {
            ...
            for (int i = startCol; i < sheet.Columns.Count; i++)
            {
                ...
                var fieldType = sheet.Rows[_setting.TypeRowNum][i].ToString();
                if (string.IsNullOrWhiteSpace(fieldName)) continue;
                rowHeads.Add(new RowHead(fieldName, fieldType, i));
                //data.Columns.Add(fieldName, Type.GetType(fieldType));

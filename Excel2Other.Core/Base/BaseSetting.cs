...
namespace Excel2Other
{
    public class BaseSetting : ISetting
    {
       ...
       [SettingAttribute("保存路径","保存时的默认路径，为空时默认在桌面", (int)StringType.Directory)]
        ...
        /// 排除前缀
        /// </summary>
        [SettingAttribute("排除前缀", "在读取时排除以特定字符开头的Sheet名")]
        public string excludePrefix;
        /// <summary>
        /// 是否排除sheet
        /// </summary>
        [SettingAttribute("根据前缀排除Sheet", "读取Excel文件时，跳过带排除前缀的Sheet名")]
        public bool excludeSheet;

        /// <summary>
        /// 是否排除第一列（第一列第一格为sheet名）
        /// </summary>
        [SettingAttribute("以第一行第一列单元格为Sheet名", "以第一个单元格为Sheet名，并排除第一列所有内容")]

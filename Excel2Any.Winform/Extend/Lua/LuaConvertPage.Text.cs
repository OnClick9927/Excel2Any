...
{
    partial class LuaConvertPage
    {
        protected Regex LuaCommentRegex1 = new Regex(@"--.*$", RegexOptions.Multiline),
                        LuaCommentRegex2 = new Regex(@"(--\[\[.*?\]\])|(--\[\[.*)", RegexOptions.Singleline),
                        LuaCommentRegex3 = new Regex(@"(--\[\[.*?\]\])|(.*\]\])", RegexOptions.Singleline | RegexOptions.RightToLeft);

        protected Regex LuaKeywordRegex = new Regex(@"\b(and|break|do|else|elseif|end|false|for|function|if|in|local|nil|not|or|repeat|return|then|true|until|while)\b");
        protected Regex LuaNumberRegex = new Regex(@"\b\d+[\.]?\d*([eE]\-?\d+)?[lLdDfF]?\b|\b0x[a-fA-F\d]+\b");
        protected Regex LuaStringRegex = new Regex(@"""""|''|"".*?[^\\]""|'.*?[^\\]'");
        protected Regex LuaFunctionsRegex = new Regex(@"\b(assert|collectgarbage|dofile|error|getfenv|getmetatable|ipairs|load|loadfile|loadstring|module|next|pairs|pcall|print|rawequal|rawget|rawset|require|select|setfenv|setmetatable|tonumber|tostring|type|unpack|xpcall)\b");
        ...
        protected void txtCode_TextChanged(object sender, TextChangedEventArgs e)
        {
